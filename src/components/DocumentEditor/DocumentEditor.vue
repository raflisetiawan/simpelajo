    ...
    <div class="editor" ref="editor">
        ...
        <div v-if="overlay" class="overlays">
            <div v-for="(page, page_idx) in pages" class="overlay" :key="page.uuid + '-overlay'"
                :ref="(elt) => (pages_overlay_refs[page.uuid] = elt)" v-html="overlay(page_idx + 1, pages.length)"
        ...
        <div class="content" ref="content" :contenteditable="editable" :style="page_style(-1)" @input="input"
            @keyup="process_current_text_style" @keydown="keydown">
            <!-- Contains every page of the document (can be modified by the DOM afterwards) -->
            <div v-for="(page, page_idx) in pages" class="page" :key="page.uuid"
                :ref="(elt) => (pages_refs[page.uuid] = elt)" :data-content-idx="page.content_idx"
                :data-page-idx="page_idx" :style="page_style(page_idx, page.template ? false : true)"
...
<script>
import { move_children_forward_recursively, move_children_backwards_with_merging } from '../../utils/page-transition-mgmt';
...
@import "../../utils/doc-editor-default-styles.scss";
</style>